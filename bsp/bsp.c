...
#define	SW_UP GPIO_Pin_4
#define	SW_LEFT GPIO_Pin_5
#define	SW_DOWN GPIO_Pin_6
#define	SW_RIGHT GPIO_Pin_2
...
/* Leds disponibles */
uint16_t const leds[] = {LED_1, LED_2, LED_3, LED_4, LED_5, LED_6, LED_7, LED_8, LED_V, LED_N, LED_R, LED_A};

/* Puertos de los switches disponibles */
GPIO_TypeDef* sw_port[] = {GPIOE, GPIOE, GPIOE, GPIOE};
/* switches disponibles */
uint16_t const sw[] = {SW_UP, SW_LEFT, SW_DOWN, SW_RIGHT};
...
uint8_t sw_getState(uint8_t boton) {
	return GPIO_ReadInputDataBit(sw_port[boton], sw[boton]);
}
...
void bsp_sw_init() {
	...
	GPIO_InitStruct.GPIO_Pin = GPIO_Pin_4 | GPIO_Pin_5 | GPIO_Pin_6 | GPIO_Pin_2;
	GPIO_InitStruct.GPIO_Mode = GPIO_Mode_IN;
	GPIO_InitStruct.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStruct.GPIO_OType = GPIO_OType_PP;
...
uint16_t read_adc() {
	...
	// Espera por el fin de la conversión
	while (ADC_GetFlagStatus(ADC1, ADC_FLAG_EOC) != SET);
	// Devolución de la medición
